...
static BOOL validateProgram(GLuint prog)
{
    ...
    if (logLength > 0)
    {
        ...
        KxDDLogCInfo(@"Program validate log:\n%s", log);
        free(log);
    }
#endif
    ...
    if (status == GL_FALSE) {
		KxDDLogCError(@"Failed to validate program %d", prog);
        return NO;
    }
...
static GLuint compileShader(GLenum type, NSString *shaderString)
{
    ...
    if (shader == 0 || shader == GL_INVALID_ENUM) {
        KxDDLogCError(@"Failed to create shader %d", type);
        return 0;
    }
    ...
    if (logLength > 0)
    {
        ...
        KxDDLogCInfo(@"Shader compile log:\n%s", log);
        free(log);
    }
#endif
    ...
    if (status == GL_FALSE) {
		...
		KxDDLogCError(@"Failed to compile shader:\n");
        return 0;
    }
             ...
             decoder: (KxMovieDecoder *) decoder
{
    ...
    if (self) {
        ...
        if ([decoder setupVideoFrameFormat:KxVideoFrameFormatYUV]) {
            ...
            KxDDLogVerbose(@"OK use YUV GL renderer");
            
        } else {
            
            _renderer = [[KxMovieGLRenderer_RGB alloc] init];
            KxDDLogVerbose(@"OK use RGB GL renderer");
        }
        ...
        if (!_context ||
            ...
            KxDDLogError(@"failed to setup EAGLContext");
            self = nil;
            return nil;
        }
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            ...
            KxDDLogError(@"failed to make complete framebuffer object %x", status);
            self = nil;
            return nil;
        }
        ...
        if (GL_NO_ERROR != glError) {
            ...
            KxDDLogError(@"failed to setup GL %x", glError);
            self = nil;
            return nil;
        }
...
- (void)layoutSubviews
{
	...
	if (status != GL_FRAMEBUFFER_COMPLETE) {
        ...
        KxDDLogVerbose(@"OK setup GL framebuffer %d:%d", _backingWidth, _backingHeight);
    }
...
- (BOOL)loadShaders
{
    ...
    if (status == GL_FALSE) {
		KxDDLogError(@"Failed to link program %d", _program);
        goto exit;
    }
...
exit:
    ...
    if (result) {
        ...
        KxDDLogVerbose(@"OK setup GL programm");
...
{        
    ...
    if ([_renderer prepareRender]) {
        ...
        {
            KxDDLogError(@"Failed to validate program");
            return;
        }
    #endif
